...
class Lip_Sitemap_Model_Sitemap extends Mage_Sitemap_Model_Sitemap
{
    ...
    {
        $enabled = (bool) Mage::getStoreConfig('lip_sitemap/general/enabled');
        if(!$enabled) {
            return parent::generateXml();
        }
        $helper = Mage::helper('lip_sitemap');

        $limit = (int) Mage::getStoreConfig('lip_sitemap/general/limit');
        if ($limit == 0) {
